...
func (e Entrypointer) Go() (runErr error) {
	...
	if e.ResultExtractionMethod == ResultExtractionMethodTerminationMessage {
		if err := e.appendArtifactOutputs(&output); err != nil {
			return err
		}
	}
...
func (e Entrypointer) appendArtifactOutputs(output *[]result.RunResult) error {
	// step artifacts
	...
	if err != nil {
		return fmt.Errorf("error while handling step artifacts: %w", err)
	}
	*output = append(*output, artifacts...)
	...
	if err != nil {
		return fmt.Errorf("error while handling task artifacts: %w", err)
	}
	*output = append(*output, artifacts...)
	return nil
}
...
// writeTerminationMessage writes results to the termination message path,
// using compression if enabled.
