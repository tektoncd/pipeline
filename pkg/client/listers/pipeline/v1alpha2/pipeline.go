...
type PipelineLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.Pipeline, err error)
...
// List lists all Pipelines in the indexer.
func (s *pipelineLister) List(selector labels.Selector) (ret []*v1alpha2.Pipeline, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.Pipeline))
	})
	return ret, err
}
...
type PipelineNamespaceLister interface {
	...
	Get(name string) (*v1alpha2.Pipeline, error)
...
// List lists all Pipelines in the indexer for a given namespace.
func (s pipelineNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.Pipeline, err error) {
...
// Get retrieves the Pipeline from the indexer for a given namespace and name.
func (s pipelineNamespaceLister) Get(name string) (*v1alpha2.Pipeline, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("pipeline"), name)
	}
	return obj.(*v1alpha2.Pipeline), nil
}
