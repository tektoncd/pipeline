...
type ConditionLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.Condition, err error)
...
// List lists all Conditions in the indexer.
func (s *conditionLister) List(selector labels.Selector) (ret []*v1alpha2.Condition, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.Condition))
	})
	return ret, err
}
...
type ConditionNamespaceLister interface {
	...
	Get(name string) (*v1alpha2.Condition, error)
...
// List lists all Conditions in the indexer for a given namespace.
func (s conditionNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.Condition, err error) {
...
// Get retrieves the Condition from the indexer for a given namespace and name.
func (s conditionNamespaceLister) Get(name string) (*v1alpha2.Condition, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("condition"), name)
	}
	return obj.(*v1alpha2.Condition), nil
}
