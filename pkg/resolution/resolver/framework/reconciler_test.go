...
var (
	...
	_ framework.TimedResolution = &controlledResolver{}
)
...
func (r *controlledResolver) ValidateParams(context.Context, []pipelinev1.Param) error {
	return nil
}

func (r *controlledResolver) GetSelector(context.Context) map[string]string {
	return map[string]string{resolutioncommon.LabelKeyResolverType: framework.LabelValueFakeResolverType}
}
...
func (r *controlledResolver) Resolve(context.Context, []pipelinev1.Param) (framework.ResolvedResource, error) {
	close(r.started)
	<-r.release
	return &framework.FakeResolvedResource{Content: `{"apiVersion": "tekton.dev/v1", "kind": "Pipeline"}`}, nil
}
...
// reverting them to unbuffered makes the worker block, which this test catches
// by polling for its stack frame. Deterministic, unlike a NumGoroutine count.
func TestResolveGoroutineLeak(t *testing.T) {
	...
	d := test.Data{ResolutionRequests: []*v1beta1.ResolutionRequest{request}}
	...
	if !waitForNoGoroutine(t, "resolver/framework.(*Reconciler).resolve.func1", 2*time.Second) {
		t.Fatal("resolver worker goroutine did not exit after timeout: send on result channel blocked")
	}
}

// waitForNoGoroutine polls until no goroutine stack contains frame, or the
// timeout elapses. It targets a single named frame instead of a global count.
func waitForNoGoroutine(t *testing.T, frame string, timeout time.Duration) bool {
	t.Helper()
