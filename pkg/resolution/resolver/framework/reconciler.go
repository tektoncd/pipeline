...
func (r *Reconciler) resolve(ctx context.Context, key string, rr *v1beta1.ResolutionRequest) error {
	...
	go func() {
		resultChan <- r.asyncResolve(resolutionCtx, key, rr.Spec.Params)
	}()
...
func (r *Reconciler) asyncResolve(ctx context.Context, key string, params []pipelinev1.Param) resolveResult {
	validationError := r.resolver.ValidateParams(ctx, params)
	...
	resource, resolveErr := r.resolver.Resolve(ctx, params)
	if resolveErr != nil {
