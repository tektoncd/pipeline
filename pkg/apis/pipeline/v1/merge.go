...
func MergeStepsWithStepTemplate(template *StepTemplate, steps []Step) ([]Step, error) {
	...
	for i, s := range steps {
		...
		newStep := Step{Script: s.Script, OnError: s.OnError, Timeout: s.Timeout, StdoutConfig: s.StdoutConfig, StderrConfig: s.StderrConfig, Results: s.Results, Params: s.Params, When: s.When}
