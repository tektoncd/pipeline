...
func TestComputeResourceRequirements_UnmarshalJSON(t *testing.T) {
	tests := []struct {
		...
		name: "numeric values (int and float)",
		json: `{"requests":{"cpu":500,"memory":"5000Gi"},"limits":{"cpu":0.5}}`,
		want: ComputeResourceRequirements{
			Requests: corev1.ResourceList{
				corev1.ResourceCPU:    resource.MustParse("500"),
				corev1.ResourceMemory: resource.MustParse("5000Gi"),
			},
			Limits: corev1.ResourceList{
...
func TestComputeResourceRequirements_Merge(t *testing.T) {
	tests := []struct {
		...
		name: "override replaces parsed with parsed",
		base: ComputeResourceRequirements{
			Requests: corev1.ResourceList{
				corev1.ResourceMemory: resource.MustParse("128Mi"),
				corev1.ResourceCPU:    resource.MustParse("100m"),
		...
		name: "override preserves raw variable references in base",
		base: ComputeResourceRequirements{
			RawRequests: map[corev1.ResourceName]string{
				corev1.ResourceMemory: "$(params.MEM)",
				corev1.ResourceCPU:    "$(params.CPU)",
		...
		name: "override with raw replaces parsed",
		base: ComputeResourceRequirements{
			Requests: corev1.ResourceList{
				corev1.ResourceMemory: resource.MustParse("128Mi"),
			},
		...
		name: "limits merge independently from requests",
		base: ComputeResourceRequirements{
			Requests: corev1.ResourceList{
				corev1.ResourceCPU: resource.MustParse("100m"),
			},
