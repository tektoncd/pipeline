...
func TestComputeResourceRequirements_UnmarshalNumericValues(t *testing.T) {
	// Kubernetes allows bare numbers for resource quantities (e.g. cpu: 500)
	jsonData := `{"requests":{"cpu":500,"memory":"128Mi"},"limits":{"cpu":"1","memory":5000}}`
	var cr ComputeResourceRequirements
	if err := json.Unmarshal([]byte(jsonData), &cr); err != nil {
		t.Fatalf("Unmarshal error: %v", err)
	}
	if cr.Requests.Cpu().String() != "500" {
		t.Errorf("expected cpu request 500, got %s", cr.Requests.Cpu())
	}
	if cr.Requests.Memory().String() != "128Mi" {
		t.Errorf("expected memory request 128Mi, got %s", cr.Requests.Memory())
	}
	if cr.Limits.Cpu().String() != "1" {
		t.Errorf("expected cpu limit 1, got %s", cr.Limits.Cpu())
	}
	if cr.Limits.Memory().String() != "5k" {
		t.Errorf("expected memory limit 5000, got %s", cr.Limits.Memory())
	}
	if cr.HasUnresolvedReferences() {
		t.Error("numeric values should not be unresolved references")
	}
}
...
func TestComputeResourceRequirements_ApplyReplacements_InvalidQuantity(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "not valid quantities") {
		t.Errorf("expected 'not valid quantities' error, got: %v", err)
	}
}
...
func TestComputeResourceRequirements_Validate_VariableReferenceForms(t *testing.T) {
	tests := []struct {
		name    string
		value   string
		wantErr bool
	}{
		{name: "plain reference", value: "$(params.MEM)"},
		{name: "reference with a quantity suffix", value: "$(params.SIZE)Mi"},
		{name: "reference with a prefix and a suffix", value: "1$(params.SIZE)0Mi"},
		{name: "bracket notation", value: `$(params["my.mem"])`},
		{name: "result reference", value: "$(tasks.a.results.mem)"},
		{name: "two references", value: "$(params.A)$(params.B)"},
		{name: "unclosed reference", value: "$(params.MEM", wantErr: true},
		{name: "second reference unclosed", value: "$(params.A)$(params.B", wantErr: true},
		{name: "not a reference at all", value: "notaref", wantErr: true},
		{name: "empty", value: "", wantErr: true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			r := ComputeResourceRequirements{
				RawRequests: map[corev1.ResourceName]string{corev1.ResourceMemory: tt.value},
			}
			if errs := r.Validate("computeResources"); (errs != nil) != tt.wantErr {
				t.Errorf("Validate(%q) error = %v, wantErr %v", tt.value, errs, tt.wantErr)
			}
		})
	}
}

func TestComputeResourceRequirements_Merge(t *testing.T) {
	tests := []struct {
		name     string
		base     ComputeResourceRequirements
		override ComputeResourceRequirements
		want     ComputeResourceRequirements
	}{{
		name: "override wins, other keys are kept",
		base: ComputeResourceRequirements{
			Requests: corev1.ResourceList{
				corev1.ResourceCPU:    resource.MustParse("100m"),
				corev1.ResourceMemory: resource.MustParse("128Mi"),
		...
		name: "raw values from the base survive the merge",
		base: ComputeResourceRequirements{
		...
		name: "raw values from the override survive the merge",
		base: ComputeResourceRequirements{
		...
		name: "a raw override replaces a parsed base value",
		base: ComputeResourceRequirements{
		...
		name: "a parsed override replaces a raw base value",
		base: ComputeResourceRequirements{
			RawRequests: map[corev1.ResourceName]string{corev1.ResourceMemory: "$(params.MEM)"},
		},
		override: ComputeResourceRequirements{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if d := cmp.Diff(tt.want, got); d != "" {
				t.Errorf("Merge() diff (-want +got): %s", d)
			}
		})
	}
}
