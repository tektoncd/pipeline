...
func schema_pkg_apis_pipeline_v1_PipelineTask(ref common.ReferenceCallback) common.OpenAPIDefinition {
	return common.OpenAPIDefinition{
			...
			SchemaProps: spec.SchemaProps{
				...
				Properties: map[string]spec.Schema{
					...
					"stepSpecs": {
						...
						SchemaProps: spec.SchemaProps{
							Description: "StepSpecs is used to override the compute resources of steps in the referenced Task at the Pipeline level. These values take precedence over the Task's step resources but can be overridden by PipelineRun's taskRunSpecs[].stepSpecs. When PipelineRun specifies stepSpecs for this task, it replaces the entire Pipeline-level list (not a per-step merge). For matrixed PipelineTasks, the same overrides apply to all fan-out TaskRuns. These fields are only meaningful for regular tasks, not custom tasks.",
							Type:        []string{"array"},
							Items: &spec.SchemaOrArray{
								Schema: &spec.Schema{
									SchemaProps: spec.SchemaProps{
					...
					"sidecarSpecs": {
						...
						SchemaProps: spec.SchemaProps{
							Description: "SidecarSpecs is used to override the compute resources of sidecars in the referenced Task at the Pipeline level. These values take precedence over the Task's sidecar resources but can be overridden by PipelineRun's taskRunSpecs[].sidecarSpecs. When PipelineRun specifies sidecarSpecs for this task, it replaces the entire Pipeline-level list (not a per-sidecar merge). These fields are only meaningful for regular tasks, not custom tasks.",
							Type:        []string{"array"},
							Items: &spec.SchemaOrArray{
								Schema: &spec.Schema{
									SchemaProps: spec.SchemaProps{
					...
					"computeResources": {
						SchemaProps: spec.SchemaProps{
							Description: "Compute resources to use for this PipelineTask. These values take precedence over the Task's compute resources but can be overridden by PipelineRun's taskRunSpecs[].computeResources. Cannot be used together with stepSpecs that set per-step resources (mutually exclusive). These fields are only meaningful for regular tasks, not custom tasks.",
		...
		Dependencies: []string{
			"github.com/tektoncd/pipeline/pkg/apis/pipeline/v1.EmbeddedTask", "github.com/tektoncd/pipeline/pkg/apis/pipeline/v1.Matrix", "github.com/tektoncd/pipeline/pkg/apis/pipeline/v1.Param", "github.com/tektoncd/pipeline/pkg/apis/pipeline/v1.PipelineRef", "github.com/tektoncd/pipeline/pkg/apis/pipeline/v1.PipelineSpec", "github.com/tektoncd/pipeline/pkg/apis/pipeline/v1.TaskRef", "github.com/tektoncd/pipeline/pkg/apis/pipeline/v1.TaskRunSidecarSpec", "github.com/tektoncd/pipeline/pkg/apis/pipeline/v1.TaskRunStepSpec", "github.com/tektoncd/pipeline/pkg/apis/pipeline/v1.WhenExpression", "github.com/tektoncd/pipeline/pkg/apis/pipeline/v1.WorkspacePipelineTaskBinding", "k8s.io/api/core/v1.ResourceRequirements", "k8s.io/apimachinery/pkg/apis/meta/v1.Duration"},
	}
}
