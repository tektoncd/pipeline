...
func TestNewResultReferenceWhenExpressions(t *testing.T) {
	type args struct {
		whenExpression v1beta1.WhenExpression
	}
	tests := []struct {
		name string
		args args
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			expressions, ok := tt.args.whenExpression.GetVarSubstitutionExpressionsForWhenExpression()
			if !ok && tt.want != nil {
				t.Fatalf("expected to find expressions but didn't find any")
			} else {
				got := v1beta1.NewResultRefs(expressions)
...
func TestLooksLikeResultRefWhenExpression(t *testing.T) {
	type args struct {
		whenExpression v1beta1.WhenExpression
	}
	tests := []struct {
