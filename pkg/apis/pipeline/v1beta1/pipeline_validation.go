...
func validatePipelineTaskParameterUsage(tasks []PipelineTask, params ParamSpecs) (errs *apis.FieldError) {
	allParamNames := sets.New[string](params.getNames()...)
	_, arrayParams, objectParams := params.sortByType()
	arrayParamNames := sets.New[string](arrayParams.getNames()...)
...
func getPipelineTasksNames(pipelineTasks []PipelineTask) sets.Set[string] {
	pipelineTaskNames := make(sets.Set[string])
...
func validatePipelineRefResultReferencesDisallowed(tasks []PipelineTask, finally []PipelineTask) (errs *apis.FieldError) {
	pipelineRefTasks := sets.New[string]()
	for _, t := range tasks {
...
func validateVarSubstitutionExpressions(tasks []PipelineTask, fieldPath string) (errs *apis.FieldError) {
	validPrefixes := sets.New[string]("params", "tasks", "finally", "context", "workspaces", "results")
	for idx, task := range tasks {
...
func (ps *PipelineSpec) GetIndexingReferencesToArrayParams() sets.Set[string] {
	...
	return sets.New[string](arrayIndexParamRefs...)
}
