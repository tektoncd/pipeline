...
var (
	...
	outputTask = tb.Task("test-output-task", "foo", tb.TaskSpec(
		...
		tb.TaskOutputs(tb.OutputsResource(storageResource.Name, v1alpha1.PipelineResourceTypeStorage)),
	))
	...
	storageResource = tb.PipelineResource("storage-resource", "foo", tb.PipelineResourceSpec(
		v1alpha1.PipelineResourceTypeStorage, tb.PipelineResourceSpecParam("Location", "gs://foo"), tb.PipelineResourceSpecParam("Type", "gcs"),
	))
...
func TestReconcile(t *testing.T) {
	...
	taskRunInputOutput := tb.TaskRun("test-taskrun-input-output", "foo",
		...
		tb.TaskRunSpec(
			...
			tb.TaskRunOutputs(
				tb.TaskRunOutputsResource(storageResource.Name,
	...
	d := test.Data{
		...
		PipelineResources: []*v1alpha1.PipelineResource{gitResource, anotherGitResource, imageResource, storageResource},
	}
	for _, tc := range []struct {
		...
		wantPod: tb.Pod("test-taskrun-input-output-pod-123456", "foo",
			...
			tb.PodSpec(
				...
				tb.PodRestartPolicy(corev1.RestartPolicyNever),
				getCredentialsInitContainer("j2tds"),
				getPlaceToolsInitContainer(),
				tb.PodContainer("step-git-source-git-resource-9l9zj", "override-with-git:latest",
					tb.Command(entrypointLocation),
					tb.Args("-wait_file", "/builder/downward/ready", "-post_file", "/builder/tools/0", "-wait_file_content", "-entrypoint", "/ko-app/git-init", "--",
						"-url", "https://foo.git", "-revision", "master", "-path", "/workspace/git-resource"),
					tb.WorkingDir(workspaceDir),
					tb.EnvVar("HOME", "/builder/home"),
					tb.VolumeMount("tools", "/builder/tools"),
					tb.VolumeMount("downward", "/builder/downward"),
				...
				tb.PodContainer("step-git-source-another-git-resource-mz4c7", "override-with-git:latest",
					tb.Command(entrypointLocation),
					tb.Args("-wait_file", "/builder/tools/0", "-post_file", "/builder/tools/1", "-entrypoint", "/ko-app/git-init", "--",
						"-url", "https://foobar.git", "-revision", "master", "-path", "/workspace/another-git-resource"),
					tb.WorkingDir(workspaceDir),
					tb.EnvVar("HOME", "/builder/home"),
					tb.VolumeMount("tools", "/builder/tools"),
					tb.VolumeMount("workspace", workspaceDir),
				...
				tb.PodContainer("step-upload-storage-resource-mssqb", "override-with-gsutil-image:latest",
					tb.Command(entrypointLocation),
					tb.Args("-wait_file", "/builder/tools/2", "-post_file", "/builder/tools/3", "-entrypoint", "/ko-app/gsutil", "--",
						"-args", "cp /workspace/output/storage-resource/* gs://foo"),
					tb.WorkingDir(workspaceDir),
					tb.EnvVar("HOME", "/builder/home"),
					tb.VolumeMount("tools", "/builder/tools"),
					tb.VolumeMount("workspace", workspaceDir),
				...
				tb.PodContainer("step-source-mkdir-storage-resource-78c5n", "override-with-bash-noop:latest",
					tb.Command(entrypointLocation),
					tb.Args("-wait_file", "/builder/tools/3", "-post_file", "/builder/tools/4", "-entrypoint", "/ko-app/bash", "--", "-args", "mkdir -p output-folder"),
					tb.WorkingDir(workspaceDir),
					tb.EnvVar("HOME", "/builder/home"),
				...
				tb.PodContainer("step-source-copy-storage-resource-6nl7g", "override-with-bash-noop:latest",
					tb.Command(entrypointLocation),
					tb.Args("-wait_file", "/builder/tools/4", "-post_file", "/builder/tools/5", "-entrypoint", "/ko-app/bash", "--",
						"-args", "cp -r /workspace/output/storage-resource/. output-folder"),
					tb.WorkingDir(workspaceDir),
					tb.EnvVar("HOME", "/builder/home"),
					tb.VolumeMount("test-pvc", "/pvc"),
					tb.VolumeMount("tools", "/builder/tools"),
