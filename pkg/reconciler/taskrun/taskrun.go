...
func (c *Reconciler) createPod(tr *v1alpha1.TaskRun, rtr *resources.ResolvedTaskResources) (*corev1.Pod, error) {
	...
	inputResources, err := getPipelineResourceInstances(rtr.Inputs, tr.OwnerReferences, c.KubeClientSet, c.Images)
	if err != nil {
		c.Logger.Errorf("Failed to instantiate input resources: %v", err)
		return nil, err
	}
	outputResources, err := getPipelineResourceInstances(rtr.Outputs, tr.OwnerReferences, c.KubeClientSet, c.Images)
	if err != nil {
...
func getPipelineResourceInstances(resources map[string]*v1alpha1.PipelineResource, o []metav1.OwnerReference, c kubernetes.Interface, images pipeline.Images) (map[string]v1alpha1.PipelineResourceInterface, error) {
