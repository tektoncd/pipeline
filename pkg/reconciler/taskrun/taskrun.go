...
func (c *Reconciler) Reconcile(ctx context.Context, key string) error {
	...
	if tr.IsDone() {
		...
		expirationErr := c.processTaskRunExpired(namespace, name, tr)
		merr = multierror.Append(cloudEventErr, updateErr, expirationErr)
	...
	merr = multierror.Append(merr, c.updateStatusLabelsAndAnnotations(tr, original))
	return multierror.Append(merr, c.processTaskRunExpired(namespace, name, tr)).ErrorOrNil()
}
