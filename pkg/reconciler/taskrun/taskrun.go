...
func (c *Reconciler) ReconcileKind(ctx context.Context, tr *v1.TaskRun) pkgreconciler.Event {
	...
	if tr.IsDone() {
		...
		var v1beta1TaskRun v1beta1.TaskRun
		if err := v1beta1TaskRun.ConvertFrom(context.Background(), tr); err == nil {
			// TODO: make this into an error type?
			return fmt.Errorf("conversion error")
		}

		// Try to send cloud events first
		cloudEventErr := cloudevent.SendCloudEvents(&v1beta1TaskRun, c.cloudEventClient, logger, c.Clock)
	...
	if err = c.reconcile(ctx, tr, rtr); err != nil {
		...
		if errors.Is(err, sidecarlogresults.ErrSizeExceeded) {
			...
			// TODO: needs https://github.com/tektoncd/pipeline/pull/5949
...
func (c *Reconciler) prepare(ctx context.Context, tr *v1.TaskRun) (*v1.TaskSpec, *resources.ResolvedTaskResources, error) {
	logger := logging.FromContext(ctx)
	tr.SetDefaults(ctx)
	...
	// taskRunResources, err := v1beta1.RetrieveResourcesFromV1TaskRun(*tr)
	// if err == nil {
	// 	return nil, nil, fmt.Errorf("failed retrieving v1beta1 Resources from v1 TaskRun", err)
	// }
	// if taskRunResources.Inputs != nil {
	// 	inputs = taskRunResources.Inputs
	// }
	// if taskRunResources.Outputs != nil {
	// 	outputs = taskRunResources.Outputs
	// }

	// Option B: consider converting here
	// it's because all taskSpecs and

	// TODO: maybe add a CannotConvertError
	var v1beta1TaskRun v1beta1.TaskRun
	if err := v1beta1TaskRun.ConvertFrom(ctx, tr); err == nil {
		fmt.Errorf("failed retrieving v1beta1 Resources from v1 TaskRun", v1beta1TaskRun)
	}

	if v1beta1TaskRun.Spec.Resources != nil {
		inputs = v1beta1TaskRun.Spec.Resources.Inputs
		outputs = v1beta1TaskRun.Spec.Resources.Outputs
	}
	// TODO
	...
	if err := ValidateResolvedTaskResources(ctx, tr.Spec.Params, &v1.Matrix{}, rtr); err != nil {
	...
	// TODO make sure conversion is consistent, here passed in CloudEvents
	logger.Debugf("Cloud Events: %s", v1beta1TaskRun.Status.CloudEvents)
	cloudevent.InitializeCloudEvents(&v1beta1TaskRun, rtr.Outputs)
...
func (c *Reconciler) reconcile(ctx context.Context, tr *v1.TaskRun, rtr *resources.ResolvedTaskResources) error {
	logger := logging.FromContext(ctx)
...
func (c *Reconciler) createPod(ctx context.Context, ts *v1.TaskSpec, tr *v1.TaskRun, rtr *resources.ResolvedTaskResources, workspaceVolumes map[string]corev1.Volume) (*corev1.Pod, error) {
	logger := logging.FromContext(ctx)
...
func applyParamsContextsResultsAndWorkspaces(ctx context.Context, tr *v1.TaskRun, rtr *resources.ResolvedTaskResources, workspaceVolumes map[string]corev1.Volume) (*v1.TaskSpec, error) {
