...
var (
	...
	getCredentialsInitContainer = func(suffix string, ops ...tb.ContainerOp) tb.PodSpecOp {
		actualOps := []tb.ContainerOp{
			...
			tb.VolumeMount("home", "/tekton/home"),
		}
	...
	getMkdirResourceContainer = func(name, dir, suffix string, ops ...tb.ContainerOp) tb.PodSpecOp {
		actualOps := []tb.ContainerOp{
			...
			tb.Args("-wait_file", "/tekton/downward/ready", "-post_file", "/tekton/tools/0", "-wait_file_content", "-entrypoint", "mkdir", "--", "-p", dir),
			tb.WorkingDir(workspaceDir),
			tb.EnvVar("HOME", "/tekton/home"),
		...
		wantPod: tb.Pod("test-taskrun-run-success-pod-123456", "foo",
			...
			tb.PodSpec(
				...
				tb.PodContainer("step-simple-step", "foo",
					...
					tb.Args("-wait_file", "/tekton/downward/ready", "-post_file", "/tekton/tools/0", "-wait_file_content", "-entrypoint", "/mycmd", "--"),
					tb.WorkingDir(workspaceDir),
					tb.EnvVar("HOME", "/tekton/home"),
					tb.VolumeMount("tools", "/tekton/tools"),
					tb.VolumeMount("downward", "/tekton/downward"),
		...
		wantPod: tb.Pod("test-taskrun-input-output-pod-123456", "foo",
			...
			tb.PodSpec(
				...
				tb.PodContainer("step-create-dir-git-resource-78c5n", "busybox",
					tb.Command(entrypointLocation),
					tb.Args("-wait_file", "/tekton/tools/0", "-post_file", "/tekton/tools/1", "-entrypoint", "mkdir", "--", "-p", "/workspace/git-resource"),
					tb.WorkingDir(workspaceDir),
					tb.EnvVar("HOME", "/tekton/home"),
					tb.VolumeMount("tools", "/tekton/tools"),
					tb.VolumeMount("workspace", workspaceDir),
				...
				tb.PodContainer("step-source-copy-git-resource-mssqb", "busybox",
					tb.Command(entrypointLocation),
					tb.Args("-wait_file", "/tekton/tools/1", "-post_file", "/tekton/tools/2", "-entrypoint", "cp", "--", "-r", "source-folder/.", "/workspace/git-resource"),
					tb.WorkingDir(workspaceDir),
					tb.EnvVar("HOME", "/tekton/home"),
				...
				tb.PodContainer("step-create-dir-another-git-resource-mz4c7", "busybox",
					tb.Command(entrypointLocation),
					tb.Args("-wait_file", "/tekton/tools/2", "-post_file", "/tekton/tools/3", "-entrypoint", "mkdir", "--", "-p", "/workspace/another-git-resource"),
					tb.WorkingDir(workspaceDir),
					tb.EnvVar("HOME", "/tekton/home"),
					tb.VolumeMount("tools", "/tekton/tools"),
					tb.VolumeMount("workspace", workspaceDir),
				...
				tb.PodContainer("step-source-copy-another-git-resource-9l9zj", "busybox",
					tb.Command(entrypointLocation),
					tb.Args("-wait_file", "/tekton/tools/3", "-post_file", "/tekton/tools/4", "-entrypoint", "cp", "--", "-r", "source-folder/.", "/workspace/another-git-resource"),
					tb.WorkingDir(workspaceDir),
					tb.EnvVar("HOME", "/tekton/home"),
					tb.VolumeMount("test-pvc", "/pvc"),
					tb.VolumeMount("tools", "/tekton/tools"),
				...
				tb.PodContainer("step-simple-step", "foo",
					tb.Command(entrypointLocation),
					tb.Args("-wait_file", "/tekton/tools/4", "-post_file", "/tekton/tools/5", "-entrypoint", "/mycmd", "--"),
					tb.WorkingDir(workspaceDir),
					tb.EnvVar("HOME", "/tekton/home"),
					tb.VolumeMount("tools", "/tekton/tools"),
					tb.VolumeMount("workspace", workspaceDir),
				...
				tb.PodContainer("step-source-mkdir-git-resource-j2tds", "busybox",
					tb.Command(entrypointLocation),
					tb.Args("-wait_file", "/tekton/tools/5", "-post_file", "/tekton/tools/6", "-entrypoint", "mkdir", "--", "-p", "output-folder"),
					tb.WorkingDir(workspaceDir),
					tb.EnvVar("HOME", "/tekton/home"),
					tb.VolumeMount("test-pvc", "/pvc"),
					tb.VolumeMount("tools", "/tekton/tools"),
				...
				tb.PodContainer("step-source-copy-git-resource-vr6ds", "busybox",
					tb.Command(entrypointLocation),
					tb.Args("-wait_file", "/tekton/tools/6", "-post_file", "/tekton/tools/7", "-entrypoint", "cp", "--", "-r", "/workspace/output/git-resource/.", "output-folder"),
					tb.WorkingDir(workspaceDir),
					tb.EnvVar("HOME", "/tekton/home"),
					tb.VolumeMount("test-pvc", "/pvc"),
					tb.VolumeMount("tools", "/tekton/tools"),
		...
		wantPod: tb.Pod("test-taskrun-with-taskspec-pod-123456", "foo",
			...
			tb.PodSpec(
				...
				tb.PodContainer("step-git-source-git-resource-9l9zj", "override-with-git:latest",
					tb.Command(entrypointLocation),
					tb.Args("-wait_file", "/tekton/downward/ready", "-post_file", "/tekton/tools/0", "-wait_file_content", "-entrypoint", "/ko-app/git-init", "--",
						"-url", "https://foo.git", "-revision", "master", "-path", "/workspace/workspace"),
					tb.WorkingDir(workspaceDir),
					tb.EnvVar("HOME", "/tekton/home"),
					tb.EnvVar("TEKTON_RESOURCE_NAME", "git-resource"),
