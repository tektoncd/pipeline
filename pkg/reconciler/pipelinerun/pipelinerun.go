...
func (c *Reconciler) resolvePipelineState(
	...
	for _, task := range tasks {
		...
		fn := tresources.GetVerifiedTaskFunc(ctx, c.KubeClientSet, c.PipelineClientSet, c.resolutionRequester, pr, task.TaskRef, trName, pr.Namespace, pr.Spec.TaskRunTemplate.ServiceAccountName, vp)
...
func (c *Reconciler) createRunObject(ctx context.Context, runName string, params v1.Params, rpt *resources.ResolvedPipelineTask, pr *v1.PipelineRun) (v1beta1.RunObject, error) {
	...
	for _, w := range workspaces {
		v1beta1WorkspaecBinding := v1beta1.WorkspaceBinding{}
		v1beta1WorkspaecBinding.ConvertFrom(ctx, w)
		customRunWorkspaces = append(customRunWorkspaces, v1beta1WorkspaecBinding)
	}
...
func filterRunsForPipelineRunStatus(logger *zap.SugaredLogger, pr *v1.PipelineRun, runObjects []v1beta1.RunObject) ([]string, []string, []schema.GroupVersionKind, []*v1beta1.CustomRunStatus) {
	...
	for _, runObj := range runObjects {
		...
		gvks = append(gvks, v1.SchemeGroupVersion.WithKind(customRun))
	}
