...
func (t ResolvedPipelineRunTask) isSkippedRunOnStatusMismatch() bool {
	...
	return c.IsFalse() && c.Reason == ReasonSkippedRunOnStatusMismatch
}
	...
	for i := range tasks {
		...
		if err != nil {
			...
			if pipelineRun.Status.TaskRuns[rprt.TaskRunName] != nil {
				...
				if prts.Status != nil {
					c := prts.Status.GetCondition(apis.ConditionSucceeded)
					if c != nil {
						if c.IsFalse() && c.Reason == ReasonSkippedRunOnStatusMismatch {
							tr := &v1alpha1.TaskRun{
							...
							tr.Status.SetCondition(&apis.Condition{
								...
								Reason:  ReasonSkippedRunOnStatusMismatch,
								Message: fmt.Sprintf("runOn states didn't match for Task %s in PipelineRun", rprt.PipelineTask.Name),
							})
							taskRun = tr
						}
					}
