...
func ResolvePipelineTask(
	...
	default:
		...
		// If no TaskRun exists for this task (e.g. it is skipped due to a when expression or has
		// not been scheduled yet), ResolvedTask is still nil because setTaskRunsAndResolvedTask was
		// never called. Resolve the task spec without a TaskRun so that declared result defaults are
		// available for pipeline result population regardless of whether the task actually ran.
		if rpt.ResolvedTask == nil {
			rt, err := resolveTask(ctx, nil, getTask, pipelineTask)
			if err != nil {
				return nil, err
			}
			rpt.ResolvedTask = rt
