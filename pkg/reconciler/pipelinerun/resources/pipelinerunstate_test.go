...
func TestGetNextTasks(t *testing.T) {
	tcs := []struct {
		...
		state:        taskCancelled,
		candidates:   sets.New[string]("mytask5"),
		expectedNext: []*ResolvedPipelineTask{},
	}, {
		name:         "no-customruns-started-both-candidates",
		state:        noCustomRunStartedState,
		candidates:   sets.New[string]("mytask13", "mytask14"),
		...
		candidates:   sets.New[string]("mytask22", "mytask23"),
		...
		candidates:   sets.New[string]("mytask21"),
		expectedNext: []*ResolvedPipelineTask{},
	}, {
		name:         "no-runs-started-both-candidates-matrix",
		state:        noCustomRunStartedStateMatrix,
		candidates:   sets.New[string]("mytask19", "mytask20"),
...
func TestPipelineRunState_GetFinalTasksAndNames(t *testing.T) {
	tcs := []struct {
		...
		expectedTaskNames:  sets.New[string](pts[0].Name, pts[1].Name),
	}, {
		// tasks: [ mytask1]
		// finally: [mytask2]
		...
		expectedFinalTasks: PipelineRunState{},
		expectedFinalNames: sets.New[string](pts[1].Name),
		expectedTaskNames:  sets.New[string](pts[0].Name),
	}, {
		// tasks: [ mytask1]
		// finally: [mytask2]
...
func TestPipelineRunState_GetResultsFuncs(t *testing.T) {
	state := PipelineRunState{{
		...
		// Cancelled task with results - isFailure() returns true for cancelled tasks
		// (ConditionSucceeded=False), so these results should be included.
		TaskRunNames: []string{"cancelled-task"},
		PipelineTask: &v1.PipelineTask{
		...
		TaskRuns: []*v1.TaskRun{{
			Status: v1.TaskRunStatus{
				...
				TaskRunStatusFields: v1.TaskRunStatusFields{
					Results: []v1.TaskRunResult{{
						Name:  "cancel-foo",
						Value: *v1.NewStructuredValues("cancel-oof"),
					}},
				},
			},
		}},
	}, {
		// Timed-out task with results - isFailure() returns true for timed-out tasks
		// (ConditionSucceeded=False), so these results should be included.
		TaskRunNames: []string{"timedout-task"},
		PipelineTask: &v1.PipelineTask{
			Name: "timedout-task-1",
		},
		TaskRuns: []*v1.TaskRun{{
			Status: v1.TaskRunStatus{
				Status: duckv1.Status{Conditions: []apis.Condition{{
				...
				TaskRunStatusFields: v1.TaskRunStatusFields{
					Results: []v1.TaskRunResult{{
						Name:  "timeout-foo",
						Value: *v1.NewStructuredValues("timeout-oof"),
					}},
				},
			},
		}},
	...
	expectedTaskResults := map[string][]v1.TaskRunResult{
		...
		// Cancelled and timed-out tasks are included because isFailure() returns true
		// for all ConditionSucceeded=False states. This is intentional.
		"cancelled-task-1": {{
			Name:  "cancel-foo",
			Value: *v1.NewStructuredValues("cancel-oof"),
		}},
