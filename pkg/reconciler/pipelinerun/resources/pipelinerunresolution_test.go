...
func makeSkippedAsStateConflicted(tr v1alpha1.TaskRun) *v1alpha1.TaskRun {
	newTr := newTaskRun(tr)
	newTr.Status.Conditions[0].Status = corev1.ConditionFalse
	newTr.Status.Conditions[0].Reason = ReasonSkippedAsStateConflicted
	return newTr
}
...
var runOnSuccessConflictedState = PipelineRunState{{
	...
	PipelineTask: &pts[11],
	TaskRunName:  "pipelinerun-mytask2",
	TaskRun:      makeSkippedAsStateConflicted(trs[2]),
	ResolvedTaskResources: &resources.ResolvedTaskResources{
		TaskSpec: &task.Spec,
	},
}}
