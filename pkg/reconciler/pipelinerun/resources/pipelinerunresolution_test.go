...
func TestIsFailure(t *testing.T) {
	for _, tc := range []struct {
		...
		rpt: ResolvedPipelineTask{
			PipelineTask: &v1beta1.PipelineTask{Name: "task", Retries: 1},
			CustomTask:   true,
			RunObject:    withCustomRunRetries(makeCustomRunFailed(customRuns[0])),
		},
		want: true,
	}, {
		...
		rpt: ResolvedPipelineTask{
			PipelineTask: &v1beta1.PipelineTask{Name: "task", Retries: 1},
			RunObject:    withCustomRunCancelled(withCustomRunRetries(makeCustomRunFailed(customRuns[0]))),
			CustomTask:   true,
		},
		want: true,
	}, {
...
func TestResolvePipelineRun_CustomTask(t *testing.T) {
	...
	for _, task := range pts {
		ps, _, err := ResolvePipelineTask(ctx, pr, nopGetTask, nopGetTaskRun, getRun, task)
		if err != nil {
...
func TestResolvePipelineRun_PipelineTaskHasNoResources(t *testing.T) {
	...
	pipelineState := PipelineRunState{}
	for _, task := range pts {
		ps, _, err := ResolvePipelineTask(context.Background(), pr, getTask, getTaskRun, nopGetRun, task)
		if err != nil {
...
func TestResolvePipelineRun_TaskDoesntExist(t *testing.T) {
	...
	for _, pt := range pts {
		_, _, err := ResolvePipelineTask(context.Background(), pr, getTask, getTaskRun, nopGetRun, pt)
...
func TestResolvePipeline_WhenExpressions(t *testing.T) {
	...
	getTask := func(_ context.Context, name string) (*v1beta1.Task, *v1beta1.RefSource, *trustedresources.VerificationResult, error) {
		return task, nil, nil, nil
	}
	pr := v1beta1.PipelineRun{
		ObjectMeta: metav1.ObjectMeta{
...
func TestIsCustomTask(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			cfg := config.NewStore(logtesting.TestLogger(t))
			ctx = cfg.ToContext(ctx)
			rpt, _, err := ResolvePipelineTask(ctx, pr, getTask, getTaskRun, getRun, tc.pt)
			if err != nil {
				t.Fatalf("Did not expect error when resolving PipelineRun: %v", err)
			}
...
func TestResolvePipelineRunTask_WithMatrixedCustomTask(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			rpt, _, err := ResolvePipelineTask(ctx, pr, getTask, getTaskRun, tc.getRun, tc.pt)
			if err != nil {
				t.Fatalf("Did not expect error when resolving PipelineRun: %v", err)
			}
			if d := cmp.Diff(tc.want, rpt); d != "" {
