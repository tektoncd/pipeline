		...
		if pipelineTaskName != "" && resultName != "" {
			for _, pt := range facts.State {
				if pt.PipelineTask.Name == pipelineTaskName {
					...
					// A matrixed task that did not run has no TaskRuns to aggregate. Its
					// declared default is resolved once per combination, so report that
					// count rather than zero to stay consistent with the resolved value.
					if resultLength == 0 && facts.EnableDefaultResults {
						resultLength = defaultedMatrixResultLength(pt, resultName)
					}
...
// defaultedMatrixResultLength returns the number of values that a matrixed PipelineTask's
// declared result default resolves to, or 0 when the task is not matrixed or the result
// declares no default. It mirrors matrixDefaultValue so that
// $(tasks.<pipelineTaskName>.matrix.<resultName>.length) agrees with the array that
// $(tasks.<pipelineTaskName>.results.<resultName>[*]) resolves to.
func defaultedMatrixResultLength(rpt *ResolvedPipelineTask, resultName string) int {
	if rpt.PipelineTask == nil || !rpt.PipelineTask.IsMatrixed() {
		return 0
	}
	if rpt.ResolvedTask == nil || rpt.ResolvedTask.TaskSpec == nil {
		return 0
	}
	for _, r := range rpt.ResolvedTask.TaskSpec.Results {
		if r.Name != resultName || r.Default == nil {
			continue
		}
		// A matrixed task may only emit results of type string, so only a string default
		// resolves to a value. Array and object defaults are rejected by matrixDefaultValue
		// and contribute no aggregated values.
		if r.Default.Type != v1.ParamTypeString {
			return 0
		}
		if combinations := rpt.PipelineTask.Matrix.CountCombinations(); combinations > 0 {
			return combinations
		}
		return 1
	}
