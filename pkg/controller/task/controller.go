...
const (
	...
	// ErrResourceExists is used as part of the Event 'reason' when a Task fails
...
// Controller is the controller implementation for Task resources
...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) error {
	...
	c.logger.Info("Starting Task controller")

	// Wait for the caches to be synced before starting workers
	c.logger.Info("Waiting for informer caches to sync")
	if ok := cache.WaitForCacheSync(stopCh, c.tasksSynced); !ok {
		return fmt.Errorf("failed to wait for caches to sync")
	}

	c.logger.Info("Starting workers")
	// Launch two workers to process Task resources
...
// enqueueTemplate takes a Task resource and converts it into a
// namespace/name string which is then put onto the work queue. This method
// should *not* be passed resources of any type other than Task.
