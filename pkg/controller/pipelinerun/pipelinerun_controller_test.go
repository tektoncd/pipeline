...
func TestReconcile(t *testing.T) {
	expectedRequest := reconcile.Request{NamespacedName: types.NamespacedName{Name: test.PipelineRunName, Namespace: "default"}}
	depKey := types.NamespacedName{Name: test.PipelineRunName + "-deployment", Namespace: "default"}
	...
	if err := test.DecodeTypeFromYamlSample(test.PipelineRunFile, instance); err != nil {
		t.Fatalf("couldn't load resource from %s: %s", test.PipelineRunFile, err)
	}

	// Create the object and expect the Reconcile and Deployment to be created
	c := context.Background()
	if err := client.Create(c, instance); err != nil {
		t.Fatalf("Failed to create instance of resource %s: %s", test.PipelineRunName, err)
	}
	...
	if err := client.Delete(c, deploy); err != nil {
		t.Errorf("Failed to delete the deployment for %s: %s", test.PipelineRunName, err)
	}
