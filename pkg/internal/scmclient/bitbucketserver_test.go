...
func TestBitbucketServer_GetFileContent_BearerAuth(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Bearer auth — bare token, project/repository level
...
func TestBitbucketServer_GetFileContent_BasicAuth(t *testing.T) {
	fileContent := []byte("apiVersion: tekton.dev/v1\nkind: Task\n")

	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Basic auth — user-level HTTP access token
	...
	client := newBitbucketServerClient(server.URL, "test_user:test_token")
	got, err := client.GetFileContent(context.Background(), "test_project", "test_repo", "tasks/build.yaml", "main")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestBitbucketServer_splitUsernamePassword(t *testing.T) {
	tests := []struct {
		token        string
		wantUsername string
		wantPassword string
		...
		{"test_user:test_token", "test_user", "test_token", true},
		{"test_user:token:with:colons", "test_user", "token:with:colons", true},
		{"baretoken", "", "baretoken", false},
	}
	for _, tt := range tests {
		username, password := splitUsernamePassword(tt.token)
		useBasic := strings.Contains(tt.token, ":")
		if username != tt.wantUsername {
			t.Errorf("splitUsernamePassword(%q) username = %q, want %q", tt.token, username, tt.wantUsername)
		}
		if password != tt.wantPassword {
			t.Errorf("splitUsernamePassword(%q) password = %q, want %q", tt.token, password, tt.wantPassword)
		}
		if useBasic != tt.wantBasic {
			t.Errorf("splitUsernamePassword(%q) useBasic = %v, want %v", tt.token, useBasic, tt.wantBasic)
		}
	}
}
