...
func (r *Resolver) Resolve(ctx context.Context, req *v1beta1.ResolutionRequestSpec) (resolutionframework.ResolvedResource, error) {
	ctx, span := otel.GetTracerProvider().Tracer("resolver:bundle").Start(ctx, "resolver:bundle:Resolve")
	defer span.End()
	...
	for _, p := range req.Params {
		switch p.Name {
		case bundleresolution.ParamBundle:
			span.SetAttributes(attribute.String("resolver.bundle.url", p.Value.StringVal))
