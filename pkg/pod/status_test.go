...
func TestSetTaskRunStatusBasedOnStepStatus(t *testing.T) {
	for _, c := range []struct {
		...
		t.Run(c.desc, func(t *testing.T) {
			...
			merr := setTaskRunStatusBasedOnStepStatus(context.Background(), logger, c.ContainerStatuses, &tr, corev1.PodRunning, kubeclient, &v1beta1.TaskSpec{}, false, nil)
			if merr != nil {
...
func TestSetTaskRunStatusBasedOnStepStatus_sidecar_logs(t *testing.T) {
	for _, c := range []struct {
		...
		t.Run(c.desc, func(t *testing.T) {
			...
			merr := setTaskRunStatusBasedOnStepStatus(ctx, logger, []corev1.ContainerStatus{{}}, &tr, pod.Status.Phase, kubeclient, &v1beta1.TaskSpec{}, false, nil)
...
func TestMakeTaskRunStatusVerify(t *testing.T) {
	...
	for _, c := range []struct {
		...
		t.Run(c.desc, func(t *testing.T) {
			...
			logger, _ := logging.NewLogger("", "status")
			kubeclient := fakek8s.NewSimpleClientset()
			got, err := MakeTaskRunStatus(context.Background(), logger, tr, &c.pod, kubeclient, &v1beta1.TaskSpec{}, true, sc)
			if err != nil {
...
func TestMakeTaskRunStatus(t *testing.T) {
	for _, c := range []struct {
		...
		t.Run(c.desc, func(t *testing.T) {
			...
			logger, _ := logging.NewLogger("", "status")
			kubeclient := fakek8s.NewSimpleClientset()
			got, err := MakeTaskRunStatus(context.Background(), logger, tr, &c.pod, kubeclient, &v1beta1.TaskSpec{}, false, nil)
			if err != nil {
				t.Errorf("MakeTaskRunResult: %s", err)
			}
...
func TestMakeTaskRunStatusAlpha(t *testing.T) {
	for _, c := range []struct {
		...
		t.Run(c.desc, func(t *testing.T) {
			...
			logger, _ := logging.NewLogger("", "status")
			kubeclient := fakek8s.NewSimpleClientset()
			got, err := MakeTaskRunStatus(context.Background(), logger, tr, &c.pod, kubeclient, &c.taskSpec, false, nil)
			if err != nil {
				t.Errorf("MakeTaskRunResult: %s", err)
			}
...
func TestMakeRunStatusJSONError(t *testing.T) {
	...
	logger, _ := logging.NewLogger("", "status")
	kubeclient := fakek8s.NewSimpleClientset()
	gotTr, err := MakeTaskRunStatus(context.Background(), logger, tr, pod, kubeclient, &v1beta1.TaskSpec{}, false, nil)
	if err == nil {
