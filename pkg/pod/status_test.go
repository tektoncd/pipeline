...
func TestSetTaskRunStatusBasedOnStepStatus(t *testing.T) {
	for _, c := range []struct {
		...
		t.Run(c.desc, func(t *testing.T) {
			...
			merr := setTaskRunStatusBasedOnStepStatus(context.Background(), logger, c.ContainerStatuses, &tr, corev1.PodRunning, kubeclient, false, nil)
			if merr != nil {
...
func TestSetTaskRunStatusBasedOnStepStatus_sidecar_logs(t *testing.T) {
	for _, c := range []struct {
		...
		t.Run(c.desc, func(t *testing.T) {
			...
			merr := setTaskRunStatusBasedOnStepStatus(ctx, logger, []corev1.ContainerStatus{{}}, &tr, pod.Status.Phase, kubeclient, false, nil)
...
func TestMakeTaskRunStatusVerify(t *testing.T) {
	...
	for _, c := range []struct {
		...
		t.Run(c.desc, func(t *testing.T) {
			...
			logger, _ := logging.NewLogger("", "status")
			kubeclient := fakek8s.NewSimpleClientset()
			got, err := MakeTaskRunStatus(context.Background(), logger, tr, &c.pod, kubeclient, true, sc)
			if err != nil {
...
func TestMakeTaskRunStatus(t *testing.T) {
	for _, c := range []struct {
		...
		t.Run(c.desc, func(t *testing.T) {
			...
			logger, _ := logging.NewLogger("", "status")
			kubeclient := fakek8s.NewSimpleClientset()
			got, err := MakeTaskRunStatus(context.Background(), logger, tr, &c.pod, kubeclient, false, nil)
			if err != nil {
				t.Errorf("MakeTaskRunResult: %s", err)
			}
...
func TestMakeRunStatusJSONError(t *testing.T) {
	...
	logger, _ := logging.NewLogger("", "status")
	kubeclient := fakek8s.NewSimpleClientset()
	gotTr, err := MakeTaskRunStatus(context.Background(), logger, tr, pod, kubeclient, false, nil)
	if err == nil {
