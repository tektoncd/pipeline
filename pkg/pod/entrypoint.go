				...
				if s.Name == containerItem.Name && s.State.Running != nil {
					for j, c := range newPod.Spec.Containers {
						if c.Name == s.Name && c.Image != nopImage {
							updated = true
							fmt.Println(fmt.Sprintf("Attempting to terminate Sidecar container %s since it was not marked for exemption from ForceTermination in pod %s.", s.Name, newPod.Name))
