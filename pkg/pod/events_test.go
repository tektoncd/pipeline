...
func TestIsGenericPendingRequiresNoUsefulPodDiagnosis(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{name: "missing secret stock readiness conditions are generic", pod: &corev1.Pod{Status: corev1.PodStatus{
			ContainerStatuses: []corev1.ContainerStatus{waiting(containerWaitingReasonCreating, "")},
			Conditions: []corev1.PodCondition{
				{Type: corev1.PodReady, Status: corev1.ConditionFalse, Reason: "ContainersNotReady", Message: "containers with unready status: [step-main]"},
				{Type: corev1.ContainersReady, Status: corev1.ConditionFalse, Reason: "ContainersNotReady", Message: "containers with unready status: [step-main]"},
			},
		}}, want: true},
...
func TestLatestWarningEventKeepsUTF8AndBoundsPersistedDiagnostic(t *testing.T) {
	...
	kubeclient.PrependReactor("list", "events", func(k8stesting.Action) (bool, runtime.Object, error) {
		return true, &corev1.EventList{Items: []corev1.Event{{InvolvedObject: corev1.ObjectReference{UID: pod.UID}, Type: corev1.EventTypeWarning, Reason: "FailedMount", Message: strings.Repeat(string(rune(0x754C)), maxEventMessageLength)}}}, nil
	})
	reason, message := latestWarningEvent(t.Context(), kubeclient, pod)
