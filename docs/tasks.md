...
**Note:** This is a change from the current behavior. If a Task does not specify a default value for a particular
`Result` and fails to emit that `Result`, the associated `TaskRun` will be marked as Failed, as described in
...
**Note:** If a `Task` specifies a default value for a particular `Result` and fails to emit that `Result`, the default value will
be available as a result regardless of whether the `Task` succeeded, skipped, or failed.

Below is an example where the `TaskRun` fails because a step exits with code 1, but the finally task consumes the
default value of that result.
...
spec:
  taskSpec:
    results:
      - name: result1
        description: result1 is not produced, but its default value is available to consumers
