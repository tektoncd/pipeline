...
| `context.pipelineTask.retries`                     | The retries of this `PipelineTask`. Only substituted when the `PipelineTask` uses an inline `taskSpec`; with `taskRef`, pass the value through `params` instead. See [Using the `retries` and `retry-count` variable substitutions](pipelines.md#using-the-retries-and-retry-count-variable-substitutions).                          |
...
| `context.task.retry-count`                         | The current retry number of this `Task`. Only substituted when the `PipelineTask` uses an inline `taskSpec`; with `taskRef` the value is passed through as a literal string. To reference it from a `taskRef` task, pass the value through `params` instead. See [Using the `retries` and `retry-count` variable substitutions](pipelines.md#using-the-retries-and-retry-count-variable-substitutions). |
