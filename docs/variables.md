...
| `context.pipelineTask.retries`                     | The retries of this `PipelineTask`. For a referenced `Task`, pass this value through a `PipelineTask` param and consume it inside the `Task` as `$(params.<name>)`. See [Using the `retries` and `retry-count` variable substitutions](pipelines.md#using-the-retries-and-retry-count-variable-substitutions).                          |
