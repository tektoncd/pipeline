...
The authenticated API supports private repositories and fetches only the file at
the specified path rather than doing a full clone. It uses a thin internal HTTP
client with provider-specific implementations, supporting the following providers:
...
| Bitbucket Cloud  | `username:api_token`              | Basic                | Both parts required. Workaround for auth bug where username was previously ignored. |
| Bitbucket Cloud  | Repository/workspace access token | Bearer               | Pass bare token without username prefix.                                            |
| Bitbucket Server | `username:token`                  | Basic                | For user-level HTTP access tokens.                                                  |
...
The token secret must contain either `username:api_token` for api tokens,
or a bare repository/workspace access token for Bearer auth:
...
type: Opaque
stringData:
  token: "myusername:myapitoken" # Basic auth (api token)
  # OR
