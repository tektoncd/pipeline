...
# Run build tests.
# If there's no `build_tests` function, do:
# * `go build` on the entire repo
# * run `/hack/verify-codegen.sh`
# * check licenses in `/cmd`
function run_build_tests() {
  return 0
}

# Run unit tests.
# If there's no `unit_tests` function, run `go test` on the entire repo.
...
# If there's no `integration_tests` function, run all `test/e2e-*tests.sh`.
