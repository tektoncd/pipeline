...
function update_licenses() {
  ...
  local dst=$1
  shift
  if [ -d "${REPO_ROOT_DIR}/vendor" ]; then
    run_go_tool ./vendor/github.com/knative/test-infra/tools/dep-collector dep-collector $@ > ./${dst}
  else
    GOFLAGS="-tags=tools" run_go_tool github.com/google/go-licenses go-licenses save ./... --save_path=${dst} --force
...
function check_licenses() {
  if [ -d "${REPO_ROOT_DIR}/vendor" ]; then
    ...
    GOFLAGS="-tags=tools" run_go_tool github.com/google/go-licenses go-licenses check ./...
  fi
}
