...
func TestPipelineRunWithServiceAccounts(t *testing.T) {
	...
	for _, serviceAccount := range serviceAccounts {
		if _, err := c.KubeClient.Kube.CoreV1().ServiceAccounts(namespace).Create(&serviceAccount); err != nil {
	...
	if err := WaitForPipelineRunState(c, pipelineRun.Name, pipelineRunTimeout, PipelineRunSucceed(pipelineRun.Name), "PipelineRunSuccess"); err != nil {
		t.Fatalf("Error waiting for PipelineRun %s to finish: %s", pipelineRun.Name, err)
	}

	// Verify it used those serviceAccount
	taskRuns, err := c.TaskRunClient.List(metav1.ListOptions{LabelSelector: fmt.Sprintf("tekton.dev/pipelineRun=%s", pipelineRun.Name)})
	if err != nil {
	...
	for _, taskRun := range taskRuns.Items {
		...
		pods, err := c.KubeClient.Kube.CoreV1().Pods(namespace).List(metav1.ListOptions{LabelSelector: fmt.Sprintf("tekton.dev/taskRun=%s", taskRun.Name)})
		if err != nil {
