...
var (
	...
	debugAfterStep      = flag.Bool("debug_after_step", false, "If specified, wait for a debugger to attach after executing the step")
...
func main() {
	...
	if err := e.Go(); err != nil {
		switch t := err.(type) {
		case entrypoint.ErrorDebugBeforeStep:
			log.Println("Skipping execute step script because before step breakpoint fail-continue")
			os.Exit(1)
		case entrypoint.ErrorDebugAftetStep:
			log.Println("Skipping subsequent steps because after step breakpoint fail-continue")
			os.Exit(1)
		case skipError:
